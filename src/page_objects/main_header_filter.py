...
class HomesMainHeaderFilter(BasePage):
    ...
    def open_guests_filters(self):
        el = self.get_element_by_role("button", name="Who Add guests")
        self.click_using_locator(el)

    def is_calendar_open(self) -> bool:
        el = self.get_element_by_role("radiogroup", name="Flexible dates").locator("div")
        return self.check_locator_visibility(el)

    def open_checking_calendar(self):
        self.get_element("text=Check in").click()
